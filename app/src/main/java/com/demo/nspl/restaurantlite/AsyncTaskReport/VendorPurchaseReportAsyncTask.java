...
public class VendorPurchaseReportAsyncTask extends AsyncTask<Void, Void, List<ClsPurchaseMaster>> {
    ...
    public VendorPurchaseReportAsyncTask(String _where, TextView txt,
                                         Context context,
    ...
    @Override
    protected void onPostExecute(List<ClsPurchaseMaster> lstClsPurchaseMasters) {
        super.onPostExecute(lstClsPurchaseMasters);

        db.close();
        progressBar.setVisibility(View.GONE);

        if (lstClsPurchaseMasters != null && lstClsPurchaseMasters.size() != 0) {
            txt.setVisibility(View.GONE);
            recyclerView.setVisibility(View.VISIBLE);
            vendorPurchaseAdapter.AddItems(lstClsPurchaseMasters);
        } else {
